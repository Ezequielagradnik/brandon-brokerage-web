...
import { NHeader, SectionMark, useLang } from "../n/chrome";
import { CARRIERS, CONTACT, EASE, GOLD, GOLD_SOFT, GOLD_DEEP, HAIR, INK_MUTED, NAVY, EXTRA } from "../n/copy";
...
export default function ConceptS() {
  ...
  return (
    <div ref={pageRef} className={styles.page}>
      ...
      <NHeader lang={lang} setLang={setLang} home="/s" />
      ...
      <section id="top" ref={heroRef} className={styles.hero}>
        ...
        <motion.div style={{ position: "absolute", inset: 0, zIndex: 1, opacity: reduce ? 1 : heroFade }}>
          <HeroField />
        </motion.div>
        <motion.div className={styles.wrap} style={{ position: "relative", zIndex: 2, width: "100%", textAlign: "center", opacity: reduce ? 1 : heroFade }}>
          ...
          <h1 className={styles.display} style={{ margin: "0 auto 32px", maxWidth: 940, color: "#f5f1e8" }}>
            ...
            <motion.span style={{ display: "block", y: reduce ? 0 : lineY2 }}><MaskReveal delay={0.35}><span className={styles.displayItalic} style={{ color: GOLD_SOFT }}>{x.heroLine2}</span></MaskReveal></motion.span>
          </h1>

          <FadeIn delay={0.7}>
            <p style={{ fontSize: "clamp(16px,1.25vw,18.5px)", lineHeight: 1.7, color: "rgba(245,241,232,0.78)", maxWidth: 560, margin: "0 auto 38px" }}>{x.heroSub}</p>
          </FadeIn>
